...
this_::Type::Type(Context &&context) noexcept(true):
...
void this_::Type::write_(Interface::Message &&message) const noexcept(true) {
    try {
        ...
        namespace exception_ = parent_::parent_::utils::exception;
        for (auto const &exception: exception_::iterate()) {
            auto &&details_ = exception_::details(exception);
            ::boost::algorithm::trim(details_);
            if (! details_.empty()) ::std::cerr << ::std::move(details_)
            << ::std::endl << ::std::flush;
        }
    } catch (...) {} }
}
